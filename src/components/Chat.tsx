        ...
        <div className="flex sm:items-center justify-between py-3 border-b-2 border-gray-200">
            <div className="relative flex items-center space-x-4">
                <div className="flex flex-col leading-tight">
                    <span className="text-lg text-slate-300">What strategy would you like to develop</span>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="border-t-2 border-gray-200 px-4 pt-4 mb-2 sm:mb-0">
            <div className="relative flex">
                <input type="text" placeholder="Write your message!" className="w-full focus:outline-none focus:placeholder-gray-400 text-black placeholder-gray-600 pl-12 bg-gray-200 rounded-md py-3" />
...
const ChatMessage = (props: { children: string, isSentByCurrentUser: boolean }) => {
    return (
        <div className={`chat-message`}>
            <div className={`flex items-end ${props.isSentByCurrentUser ? "justify-end" : ""}`}>
                <div className={`flex flex-col space-y-2 text-xs mx-2 order-${props.isSentByCurrentUser ? "1" : "2"} ${props.isSentByCurrentUser ? "items-end" : "items-start"}`}>
                    <div>
                        <span className={`px-4 py-2 rounded-lg inline-block overflow-auto rounded-${props.isSentByCurrentUser ? "br" : "bl"}-none ${props.isSentByCurrentUser ? "bg-blue-600 text-white" : "bg-gray-300 text-gray-600"}`}>
                            {props.children}
                        </span>
                    </div>
                </div>
...
const ChatMessages = (props: { messages: ChatMessage[] }) => {
    return (
      <div id="messages" className="flex flex-col space-y-4 p-3 overflow-y-auto">
        {props.messages.map((message, index) => (
          <ChatMessage key={index} isSentByCurrentUser={message.isSentByCurrentUser}>
            {message.content}
          </ChatMessage>
        ))}
      </div>
...
export const Chat = (props: { messages: ChatMessage[] }) => {

    return (
        <div className="flex-1 p:2 sm:p-6 justify-between flex flex-col max-w-md">
            <ChatHeader />
